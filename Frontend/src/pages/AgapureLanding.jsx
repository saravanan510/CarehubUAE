...
const styles = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* HERO */
  .hero{background:linear-gradient(135deg,#0D2E6B 0%,#1A4F8A 35%,#1D4ED8 70%,#2563EB 100%);padding:80px 32px 0;overflow:hidden;position:relative}
  ...
  /* CTA BAND */
  .cta-band{background:linear-gradient(135deg,#0D2E6B 0%,#1D4ED8 100%);padding:72px 32px;text-align:center}
...
const HeartIcon = () => (
  <svg viewBox="0 0 24 24">
    <path d="M12 21.593c-5.63-5.539-11-10.297-11-14.402C1 3.01 3.42 1 6 1c1.75 0 3.502.81 4.5 2.344C11.5 1.809 13.252 1 15 1c2.58 0 5 2.01 5 6.191 0 4.105-5.37 8.863-11 14.402z" />
  </svg>
);
...
function App() {
  ...
  const [toastMsg, setToastMsg] = useState({ msg: "", icon: "✓", show: false });
  const [content, setContent] = useState({});
  const [allPkgs, setAllPkgs] = useState([]);
  const [filteredPkgs, setFilteredPkgs] = useState([]);
  const [activeCat, setActiveCat] = useState("all");
  const [categories, setCategories] = useState(["all"]);
  const [featuredPkg, setFeaturedPkg] = useState(null);
  ...
  useEffect(() => {
    ...
    const unsubPkgs = onValue(pkgRef, (snap) => {
      const data = snap.val();
      if (!data) {
        setAllPkgs([]);
        setFilteredPkgs([]);
      ...
      const cats = [
        "all",
        ...new Set(pkgs.map((p) => p.category).filter(Boolean)),
      ];
      setCategories(cats);

      const feat = pkgs.find((p) => p.featured) || pkgs[0] || null;
      setFeaturedPkg(feat);
    });
  ...
  const openModalWithPkg = (name) => {
    setSelectedPkg(name);
    setFormData((prev) => ({ ...prev, pkg: name }));
    openModal();
  };

  /* ── FIX: Only close when clicking the dark overlay itself, not the modal ── */
  const handleOverlayClick = (e) => {
    if (e.target === overlayRef.current) closeModal();
  };

  /* ── Filter ── */
  const filterPkgs = (cat) => {
    setActiveCat(cat);
    if (cat === "all") {
      setFilteredPkgs(allPkgs);
    } else {
      setFilteredPkgs(allPkgs.filter((p) => p.category === cat));
    }
  };
  ...
  /* ── Submit Booking ── */
  const submitBooking = () => {
    const { fname, phone, pkg } = formData;
    if (!fname.trim() || !phone.trim() || !pkg) {
      toast("Please fill in Name, Phone and Package.", "⚠️");
      return;
    }
    closeModal();
    toast(
  ...
  const getText = (key, fallback) => content[key] || fallback;
  ...
  const heroStats =
    content.heroStats && Array.isArray(content.heroStats)
      ? content.heroStats
      : defaultStats;
    ...
    <>
      ...
      <div id="loader" className={loading ? "" : "out"}>
        <div className="ld-logo">
          ...
          <div className="ld-text">{getText("siteName", "ArogyaPlus")}</div>
        </div>
      </div>
            ...
            <a href="#" className="brand">
              ...
              <div className="brand-text">
                <span className="brand-name">
                  {getText("siteName", "ArogyaPlus")}
                </span>
                <span className="brand-sub">
                  {getText("siteTagline", "Health Packages")}
                </span>
              </div>
            </a>
        ...
        <div className="hero-inner">
          <div>
            <div className="hero-badge">
              ...
              <span>
                {getText("heroEyebrow", "Trusted by 50,000+ Families")}
              </span>
            </div>
            <h1
              dangerouslySetInnerHTML={{
                __html: getText(
                  "heroTitle",
                  "Comprehensive<br><em>Health Packages</em><br>for Every Family",
                ),
              }}
            />
            ...
            <div className="hero-actions">
              <button className="btn-hero-primary" onClick={scrollToPackages}>
                View Packages
              </button>
          ...
          <div className="hero-card">
            ...
            <div>
              <div style={{ textAlign: "center", padding: "16px 0" }}>
                ...
                >
                  {featuredPkg?.name || "No Packages Yet"}
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  {(featuredPkg?.tests || []).length} tests included
                </div>
              </div>
              <div
                style={{
              ...
              >
                {(featuredPkg?.tests || []).slice(0, 4).map((t, i) => (
                  <div
                    key={i}
                    style={{
                      display: "flex",
                ...
                {(!featuredPkg?.tests || featuredPkg.tests.length === 0) && (
                  <div
                    style={{
                      color: "var(--muted)",
                      fontSize: ".8rem",
                  ...
                  >
                    Packages will appear here once added by admin.
                  </div>
                )}
              </div>
              <button
                ...
                onClick={() =>
                  featuredPkg
                    ? openModalWithPkg(featuredPkg.name)
                    : scrollToPackages()
                }
              >
                {featuredPkg
                  ? `Book ${featuredPkg.name} →`
                  : "See All Packages →"}
              </button>
            </div>
          </div>
        ...
        <div className="trust-inner">
          <div className="trust-item">
            ...
            <div className="trust-text">
              <strong>{getText("trust1", "NABH Accredited Labs")}</strong>
          ...
          <div className="trust-item">
            <div className="trust-ic">🔒</div>
            <div className="trust-text">
              <strong>{getText("trust2", "HIPAA Compliant")}</strong>
          ...
          <div className="trust-item">
            <div className="trust-ic">🚐</div>
            <div className="trust-text">
              <strong>{getText("trust3", "Free Home Collection")}</strong>
          ...
          <div className="trust-item">
            <div className="trust-ic">📱</div>
            <div className="trust-text">
              <strong>{getText("trust4", "Digital Reports in 48h")}</strong>
              <span>With doctor review</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="section-inner">
          <div className="sec-head">
            ...
            <h2 className="sec-title">
              {getText("pkgSectionTitle", "Choose Your Health Package")}
            </h2>
              ...
              filteredPkgs.map((p) => (
                ...
                >
                  ...
                  <div className="pkg-footer">
                    <button
                      className="pkg-btn"
                      onClick={() => openModalWithPkg(p.name)}
                    >
                      Book This Package
                    </button>
                  </div>
      ...
      <section className="section" id="why">
        <div className="section-inner">
          <div className="sec-head">
            ...
            <h2 className="sec-title">
              {getText("whyTitle", "Healthcare You Can Trust")}
            </h2>
            <p className="sec-desc">
              {getText(
                "whyDesc",
      ...
      <section className="section" id="how">
        <div className="section-inner">
          <div className="sec-head">
            ...
            <h2 className="sec-title">
              {getText("stepsTitle", "How It Works")}
            </h2>
            <p className="sec-desc">
      ...
      <section className="section" id="reviews">
        <div className="section-inner">
          <div className="sec-head">
            ...
            <h2 className="sec-title">
              {getText("testiTitle", "What Our Patients Say")}
            </h2>
          </div>
      ...
      <section className="cta-band">
        <div className="cta-band-inner">
          <h2>{getText("ctaTitle", "Start Your Health Journey Today")}</h2>
          <p>
            {getText(
              "ctaSubtitle",
          ...
          <div className="cta-btns">
            ...
            <button className="cta-ghost">📞 Call Us Now</button>
          </div>
        </div>
      </section>
      ...
      <footer>
        <div className="footer-inner">
          ...
          <span className="footer-copy">
            {getText("footerText", "© 2024 ArogyaPlus. All rights reserved.")}
          </span>
        </div>
      </footer>
      ...
      >
        {/* stopPropagation prevents clicks inside the modal from bubbling to the overlay */}
        <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="m-group">
            ...
            <select
              value={formData.pkg}
              onChange={(e) => handleInput("pkg", e.target.value)}
            >
          ...
          <button className="m-submit" onClick={submitBooking}>
            Confirm Booking
          </button>
        </div>
      </div>
      ...
      {/* ─── TOAST ─── */}
      <div id="toast" className={toastMsg.show ? "on" : ""}>
        <span className="t-ic">{toastMsg.icon}</span>
